...
class shield(commands.Cog):
    ...
    async def shield(self, ctx, member: nextcord.Member = None, role: nextcord.Role = None):
    ...
    @commands.has_permissions(manage_roles=True)
    async def unshield(self, ctx, member: nextcord.Member = None, role: nextcord.Role = None):
        if member == None or role == None:
            await ctx.send("Incorrect member and/or role specified")
        else:
